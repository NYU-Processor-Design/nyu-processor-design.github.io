...
This describes a 2-to-1 multiplexer. The block re-evaluates whenever a signal used inside it changes.

Older Verilog commonly uses `always @(*)`; `always_comb` is the clearer SystemVerilog form.
...
module counter_tb;
    ...
    initial begin
        ...
        if (count !== 4'd1)
            $fatal("First increment failed");

        @(posedge clk);
        #1;

        if (count !== 4'd2)
            $fatal("Second increment failed");
