...
The majority of time this week was spent working on outlines for modules, specifically for stuff like the cache managers and general control module. Uma completed the tests for the register module so now we have another functional module to add to the repo. As I expected, things are slowing down as finals approach but we are still making good progress.